...
{
    if (!jello->incPlanePresent || jello->c == 0.00) return; // Skip if no inclined plane

    glColor4f(0.5, 0.6, 0.7, 0.6); // Bluish metallic steel

    glBegin(GL_QUADS);

    // struct point p1, p2, p3, p4;

    struct point p1 = {-2, -2, (-jello->d - jello->a * -2 - jello->b * -2) / jello->c};
    struct point p2 = { 2, -2, (-jello->d - jello->a * 2 - jello->b * -2) / jello->c};
    struct point p3 = { 2,  2, (-jello->d - jello->a * 2 - jello->b * 2) / jello->c};
    struct point p4 = {-2,  2, (-jello->d - jello->a * -2 - jello->b * 2) / jello->c};
...
void doIdle()
{
  char s[30]="Frames/picxxxx.ppm";
  int i;
  // system("mkdir -p frames");

  // save screen to file
  s[10] = 48 + (sprite / 1000);
  s[11] = 48 + (sprite % 1000) / 100;
  s[12] = 48 + (sprite % 100 ) / 10;
  s[13] = 48 + sprite % 10;
  ...
  if (saveScreenToFile == 1) // Capture every 15 frames
    {
        saveScreenshot(windowWidth, windowHeight, s);
        sprite++;
    }
...
// int capturedFrames = 0; // Number of frames actually saved

// void doIdle()
// {
//     if (saveScreenToFile == 1 && sprite % 15 == 0 && capturedFrames < 300) // Stop at 300 frames
//     {
//         // std::filesystem::create_directories("frames"); // Ensure "frames" directory exists

//         // Generate filename in the format "000.ppm", "001.ppm", ..., "299.ppm"
//         char filename[30];
//         std::snprintf(filename, sizeof(filename), "frames/%03d.jpg", capturedFrames);

//         saveScreenshot(windowWidth, windowHeight, filename);
//         capturedFrames++; // Increment only when a frame is saved
//     }

//     if (capturedFrames >= 300) // Stop program after saving 300 frames
//     {
//         exit(0);
//     }
